function isoFit01(nameOfParamsFile)
...
[theriakDominoPath,...
    ...
    loopOutputTable,...
    PTloopCommandsFile...
    ] = isoFit01_readParams(nameOfParamsFile);
...
fid = fopen('isoFit.bat','wt');
fprintf(fid,'set THERDOM=%s\n',theriakDominoPath);
fprintf(fid,'set PATH=%%THERDOM%%;%%path%%\ntheriak < isoFitInput.dat\n');
fclose(fid);

fid = fopen('isoFitInput.dat','wt');
...
for n = 1: num_points
    ...
    alm_obs = garnetcomp.data(n,2);
    prp_obs = garnetcomp.data(n,3);
    sps_obs = garnetcomp.data(n,4);
    grs_obs = garnetcomp.data(n,5);
...
%     PT = [node(1,n).T,node(1,n).P];
    % For fgoalattain, we might need to be more specific about the first
    % estimate of the isopleth intersection.
    if n == 1
        PT = [550,5100]; % Add new parameters to the params file
    else
        PT = [node(1,n).T,node(1,n).P];
    end
    ...
    % Define the function
%     misfitFun = isoFit01_misfit_function(PT,T1,P1,...
...
%         'TolFun',optimsetToleranceFun,'TolX',optimsetToleranceX);
    ...
    % Some options require "legacy" names (e.g., FinDiffRelStep is a legacy
    % name for FiniteDifferenceStepSize)
    functionOptions = optimoptions(...
        'fgoalattain',...
        'Display',optimoptionsDisplay,...
        'FinDiffRelStep',0.1,... % Step size
        'GoalsExactAchieve',4,... % Number of goals to make equal (function results will be equalized if possible)
        'TolFun',optimoptionsFun,...
        'TolX',optimoptionsTolX...
        );
    

    goalForGrtChem = [0 0 0 0];
    weight = abs(goalForGrtChem);
    
    % Run the search routine
    %fminsearch(functionOfPT,PT,functionOptions);
    fgoalattain(...
        @(PT)isoFit01_misfit_function(PT,T1,P1,...
        alm_obs,prp_obs,sps_obs,grs_obs,mgNumObs,...
        loopStep,spss_c,useMgNum,useFeNum,...
        useAlmPrpGrs,useDiff,useNormalizedMisfitFun,use4EndMembers,...
        usePrpGrs,useSpsGrs,useAlmPrpSps,...
        GrtName,GrtRemovePercent,...
        therinNamePWD,loopOutputTable,PTloopCommandsFile,...
        figID2,drawCandidateFig),...
        PT,goalForGrtChem,weight,...
        [],[],[],[],[],[],[],functionOptions);
    
    % Read the new values produced from the search
    [alm_mod,prp_mod,sps_mod,grs_mod,mgNum_mod,V_SolidsCC,V_GrtCC,headers,data] = ...
    ...
    node(1,n+1).Xalm =   alm_mod;
    node(1,n+1).Xpy =    prp_mod;
    node(1,n+1).Xspss =  sps_mod;
    node(1,n+1).Xgr =    grs_mod;
    node(1,n+1).mgNumObs =       mgNumObs;
    node(1,n+1).mgNum =          mgNum_mod;
    node(1,n+1).feNumObs =       1 - mgNumObs;
    node(1,n+1).feNum =          1 - mgNum_mod;
