...
function [x, fval] = fminsearch (fun, x0, options = struct ())
  ...
  # Edits in progress (EK)
  # Maybe here, add some code that gets the centroid PT from the
  # misfit function.  Assign the new PT to x0 so that this function
  # uses the better one.
  # Will this screw up the function's ability to choose a new point
  # in the NM algorithm?  Probably not.  x0 will be used to generate
  # x in the line below (x = nmmax ...).
  
  # Why is the function called nmsmax (maximum?)?  Shouldn't it find
  # a minimum?
